...
#define APP_VERSION_STR _T("v18") // 用于显示的字符串版本
#define APP_VERSION_NUM 18        // 用于逻辑比较的数字版本
...
bool isBusy = false;
static ULONGLONG lastPowerCheckTime = 0;
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow) {
    ...
    while (g.appRunning) {
        ...
        if (g.startupState == STARTUP_PHASE_1_HIDING) { // 0: 启动时的隐藏阶段
            ...
            EnterCriticalSection(&g_csLogic);   // 加锁
            bool isIdle = IsPhysicsIdle();
            LeaveCriticalSection(&g_csLogic);   // 解锁
        ...
        if (!physicsIdle) {
            ...
            // 防御性钳制：如果省电模式导致单次间隔超过了 0.05秒，强行压回去
            // 防止弹簧引擎因为 delta time 过大而产生“爆炸”飞走
            if (dt > 0.1f) dt = 0.01f;
    ...
    while (g.appRunning) {
        ...
        if (currTime - lastEnvCheckTime > 3000) {
            ...
            if (GetSystemPowerStatus(&sps)) {
                bool isWindowsSaverToggleOn = g_isWindowsSaverToggleOn; // 绿叶子开关
                bool isOnBattery = (sps.ACLineStatus == 0);                // 纯电池供电

                if (isWindowsSaverToggleOn || isOnBattery) {
...
LRESULT CALLBACK MsgWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_ENV_CHECK_UPDATE: {
        bool isBatterySaverOn = (lParam != 0);
    ...
    case WM_POWERBROADCAST:
        ...
        if (wParam == 32787) { // 32787 即 PBT_POWERSETTINGCHANGE
...
void UpdatePhysics(float dt) {
    ...
    if (renderY != g.lastRenderY) {
        // 强制图标层在最顶层 (HWND_TOP)
        SetWindowPos(g.hContainer, HWND_TOP, 0, renderY, 0, 0, SWP_NOSIZE | SWP_NOACTIVATE);
        ...
        if (g.hMaskWindow && IsWindow(g.hMaskWindow) && IsWindowVisible(g.hMaskWindow)) {
            ...
            // 强制遮罩层紧紧垫在图标层下面 (g.hContainer)
            SetWindowPos(g.hMaskWindow, g.hContainer, 0, renderY - offsetY, g.screenW, extendedH, SWP_NOACTIVATE);
        }
