...
#define APP_VERSION_STR _T("v20") // 用于显示的字符串版本
#define APP_VERSION_NUM 20        // 用于逻辑比较的数字版本
...
#define REG_VAL_POWERSAVER_OVERRIDE _T("PowerSaverOverride")
...
bool CheckSingleUrl(const TCHAR* url, HINTERNET hSession, int& outVersion, TCHAR* outFinalUrl, size_t bufferSize) {
    ...
    DWORD dwInfoSize = (DWORD)(bufferSize * sizeof(TCHAR));
...
DWORD WINAPI CheckUpdateThreadProc(LPVOID lpParam) {
    ...
    if (hSession) {
        ...
        if (CheckSingleUrl(checkUrl, hSession, ver, finalUrl, _countof(finalUrl))) {
...
void StartUpdateChecks() {
    ...
    for (int i = 0; i < _countof(MIRRORS); i++) {
        HANDLE hThread = CreateThread(NULL, 0, CheckUpdateThreadProc, (LPVOID)MIRRORS[i], 0, NULL);
...
void ShowTrayMenu(HWND hwnd) {
    ...
    TCHAR szAnimMenuTitle[128];

    // 只有当开关开启且确实处于省电激活状态时，才置灰
    if (g.enablePowerSaver && g.isPowerSaverActive) {
        _tcscpy_s(szAnimMenuTitle, _countof(szAnimMenuTitle), _T("省电模式激活中（PowerSaver Activated）"));
        animMenuFlags |= MF_GRAYED; // 关键：添加此标志使子菜单无法被打开
    }
    else {
        _tcscpy_s(szAnimMenuTitle, _countof(szAnimMenuTitle), _T("动画模式 (Animation Mode)"));
    }

    AppendMenu(hMenu, animMenuFlags, (UINT_PTR)hSubProfile, szAnimMenuTitle);
    ...
    AppendMenu(hMenu, psFlags, ID_TRAY_POWERSAVER, _T("省电适配 (PowerSaver Adaptation)"));
...
void ApplyAnimation() {
    ...
    if (renderY != g.lastRenderY) {
        ...
        if (g.hMaskWindow && IsWindow(g.hMaskWindow) && IsWindowVisible(g.hMaskWindow)) {
            ...
            SetWindowPos(g.hMaskWindow, NULL, 0, renderY - offsetY, g.screenW, extendedH,
...
void HandleInstallation() {
    ...
    if (!IsRunAsAdministrator()) {
        ...
        int result = MessageBox(NULL,
            ...
            APP_NAME,
            MB_YESNO | MB_ICONQUESTION | MB_TOPMOST | MB_SETFOREGROUND); // <--- 关键修改
    ...
    // 定义通用的“等待窗口”创建逻辑 (带 WS_EX_TOPMOST)
    auto CreateWaitWindow = [&](const TCHAR* text) -> HWND {
        // 使用 CreateWindowEx 添加 WS_EX_TOPMOST 属性
        return CreateWindowEx(
            WS_EX_TOPMOST | WS_EX_TOOLWINDOW, // <--- 关键修改：强制置顶且不在任务栏显示
            _T("STATIC"), text,
            WS_POPUP | WS_VISIBLE | SS_CENTER | SS_CENTERIMAGE, // 增加居中对齐样式
            xPos, yPos, winW, winH, // 屏幕居中
            NULL, NULL, NULL, NULL
        );
        };
    ...
    if (!bFileExists) {
        ...
        // 关键修改：添加 MB_TOPMOST | MB_SETFOREGROUND
        if (MessageBox(NULL, msg, APP_NAME, MB_YESNO | MB_ICONQUESTION | MB_TOPMOST | MB_SETFOREGROUND) == IDYES) {
            HWND hWaitWnd = CreateWaitWindow(_T("正在安装..."));

            if (CopyToInstallDirWithRetry()) {
                RegisterUninstallInfo();
                DestroyWindow(hWaitWnd);
                MessageBox(NULL, _T("安装成功！\n\n“在任务栏托盘图标右键可以打开设置菜单”"), APP_NAME, MB_OK | MB_ICONINFORMATION | MB_TOPMOST | MB_SETFOREGROUND);
        ...
        // 关键修改：添加 MB_TOPMOST | MB_SETFOREGROUND
        if (MessageBox(NULL, msg, APP_NAME, MB_YESNO | MB_ICONASTERISK | MB_TOPMOST | MB_SETFOREGROUND) == IDYES) {
            HWND hWaitWnd = CreateWaitWindow(_T("正在更新..."));

            if (CopyToInstallDirWithRetry()) {
                RegisterUninstallInfo();
                DestroyWindow(hWaitWnd);
                MessageBox(NULL, _T("更新成功！"), APP_NAME, MB_OK | MB_ICONINFORMATION | MB_TOPMOST | MB_SETFOREGROUND);
                RunInstalledExe(); exit(0);
            }
            else { DestroyWindow(hWaitWnd); exit(1); }
        }
        ...
        // 关键修改：添加 MB_TOPMOST | MB_SETFOREGROUND
        int choice = MessageBox(NULL, msg, APP_NAME, MB_YESNOCANCEL | MB_ICONINFORMATION | MB_TOPMOST | MB_SETFOREGROUND);

        if (choice == IDYES) { RunInstalledExe(); exit(0); }
        else if (choice == IDNO) {
            HWND hWaitWnd = CreateWaitWindow(_T("正在重新安装..."));

            if (CopyToInstallDirWithRetry()) {
                RegisterUninstallInfo();
                DestroyWindow(hWaitWnd);
                MessageBox(NULL, _T("修复成功！"), APP_NAME, MB_OK | MB_ICONINFORMATION | MB_TOPMOST | MB_SETFOREGROUND);
                RunInstalledExe(); exit(0);
            }
            else { DestroyWindow(hWaitWnd); exit(1); }
        }
        ...
        // 关键修改：添加 MB_TOPMOST | MB_SETFOREGROUND
        if (MessageBox(NULL, msg, APP_NAME, MB_YESNO | MB_ICONWARNING | MB_TOPMOST | MB_SETFOREGROUND) == IDYES) {
            HWND hWaitWnd = CreateWaitWindow(_T("正在回退..."));

            if (CopyToInstallDirWithRetry()) {
                RegisterUninstallInfo();
                DestroyWindow(hWaitWnd);
                MessageBox(NULL, _T("回退成功！"), APP_NAME, MB_OK | MB_ICONINFORMATION | MB_TOPMOST | MB_SETFOREGROUND);
                RunInstalledExe(); exit(0);
            }
            else { DestroyWindow(hWaitWnd); exit(1); }
        }
...
void SaveSettings() {
    ...
    if (RegCreateKeyEx(HKEY_CURRENT_USER, REG_SUBKEY, 0, NULL, 0, KEY_WRITE, NULL, &hKey, NULL) == ERROR_SUCCESS) {
        ...
        DWORD psVal = g.enablePowerSaver ? 1 : 0;
        RegSetValueEx(hKey, REG_VAL_POWERSAVER_OVERRIDE, 0, REG_DWORD, (BYTE*)&psVal, sizeof(psVal));
        RegCloseKey(hKey);
    }
}
...
void LoadSettings() {
    ...
    g.cfgIndex = 0;
    g.maskOptIndex = 0;
    g.enablePowerSaver = true; // 默认开启省电特性
    g.isPowerSaverActive = false; // 初始实际状态为 false，等待后台线程首次检测
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, REG_SUBKEY, 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        ...
        DWORD psVal = 1;
        size = sizeof(DWORD);
        if (RegQueryValueEx(hKey, REG_VAL_POWERSAVER_OVERRIDE, NULL, NULL, (BYTE*)&psVal, &size) == ERROR_SUCCESS) {
            g.enablePowerSaver = (psVal != 0);
        }
