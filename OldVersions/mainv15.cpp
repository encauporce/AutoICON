...
#define APP_VERSION_STR _T("v15") // 用于显示的字符串版本
#define APP_VERSION_NUM 15        // 用于逻辑比较的数字版本
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow) {
    ...
    while (g.appRunning) {
        ...
        // 3: 正常运行阶段
            ...
            // 检查桌面是否处于忙碌状态（正在重命名 或 正在显示桌面右键菜单）
            bool isBusy = IsDesktopBusy();

            // 如果鼠标在移动，或者桌面正忙
            if (isMoving || isBusy) {
                // 只有当鼠标确实在桌面上，或者处于忙碌状态(重命名中/菜单中)时，才重置计时器
                if (isBusy || IsMouseOnDesktop()) {
                    g.lastActiveTime = currTime; // <--- 关键：重置计时器，防止隐藏
        ...
        // 每 3 秒检测一次环境，避免占用过多 CPU
        if (currTime - lastCheckTime > 3000) {
